...
const displayMovements = function (acc, sort = false) {
  ...
  // const dateList = acc.movementsDates.map(date => {
  //   const dateDisplay = new Date(date);
  //   const day = `${dateDisplay.getDate()}`.padStart(2, 0);
  //   const month = `${dateDisplay.getMonth() + 1}`.padStart(2, 0);
  ...
  movs.forEach(function (mov, i) {
    ...
    const currenyDisplay = formatCurrency(mov, acc.locale, acc.currency);

    const move = `<div class="movements__row">
    <div class="movements__type movements__type--${type}">${i + 1} ${type}</div>
    <div class="movements__date">${dateDisplay}</div>
    <div class="movements__value">${currenyDisplay}</div>
  </div>`;
...
const updateUI = function (account) {
  //Display Accountsmovements
  displayMovements(account);

  ////Display Accountssummary
  calcDisplaySummary(account);

  //Display Accountsbalance
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currenAccount?.pin === +inputLoginPin.value) {
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    targetAccount &&
    targetAccount?.username !== currenAccount.username &&
    amount > 0 &&
    amount <= currenAccount.balance
  ) {
    //Doing Transfer
...
btnClose.addEventListener('click', function (e) {
  ...
  if (
    ...
    currenAccount.pin === +inputClosePin.value
  ) {
